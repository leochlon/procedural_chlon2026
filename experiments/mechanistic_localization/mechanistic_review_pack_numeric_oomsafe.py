...
def build_value_token_pool(tokenizer, num_values: int, max_value_len: int) -> TokenPool:
    """
    Single-token alphanumeric value pool.
    Prefer tokens with leading whitespace so that "KEY =" + " apple" => "KEY = apple".
    """
...
def build_numeric_token_map(tokenizer, max_n: int) -> Dict[int, int]:
    """
    Map integer n -> token_id for a *single-token* string form of n.

    Different tokenizers handle whitespace differently. We therefore try several
    common textual forms and accept the first that is a single token:
      1) f" {n}"   (leading space; common in GPT-style BPE)
      2) f"{n}"    (no leading space)
      3) f"\n{n}"  (newline prefix; sometimes merged)
    """
    special = set(tokenizer.all_special_ids)
    ...
    for n in range(max_n + 1):
        for s in (f" {n}", f"{n}", f"\n{n}"):
            ...
            if len(ids) == 1 and ids[0] not in special:
                mapping[n] = ids[0]
                break
    return mapping


def build_count_codebook_map(tokenizer, max_n: int, max_len: int = 8) -> Dict[int, int]:
    """
    Fallback mapping int->token_id that does NOT require numeric tokens.

    We pick arbitrary single-token alphanumeric strings (optionally with leading whitespace)
    and use them as a codebook for counts. This keeps the counting case study in a strict
    "single-token label" setting even when the tokenizer does not represent integers as
    single tokens (this occurs for some modern tokenizers).

    Returns a mapping for ALL integers in [0, max_n].
    """
    special = set(tokenizer.all_special_ids)
    ...
    # Pass 1: strict alphanumeric (best for readability)
    for tid in range(tokenizer.vocab_size):
        if tid in special:
            continue
        s = tokenizer.decode([tid], clean_up_tokenization_spaces=False)
    ...
    # Pass 2: relax pattern but keep "short + printable + not whitespace-only"
    if len(ids) < (max_n + 1):
        for tid in range(tokenizer.vocab_size):
    ...
    if len(ids) < (max_n + 1):
        raise RuntimeError(
            f"Could not build a codebook of {max_n + 1} single-token count values; only found {len(ids)}."
        )

    return {n: ids[n] for n in range(max_n + 1)}


def build_count_token_map_auto(tokenizer, max_n: int) -> Tuple[Dict[int, int], str]:
    """
    Auto-select a count token mapping.

    If the tokenizer supports a *complete* single-token mapping for 0..max_n using
    common numeric forms, we use that ("numeric"). Otherwise we fall back to a
    single-token alphanumeric codebook ("codebook") so the counting task always runs.
    """
    num_map = build_numeric_token_map(tokenizer, max_n=max_n)
    if len(num_map) == (max_n + 1):
        return num_map, "numeric"
    return build_count_codebook_map(tokenizer, max_n=max_n), "codebook"
...
def run_behavior_curve(
    ...
    for k in tqdm(distances, desc="Behavior"):
        ...
        rows.append({"k": k, "n": n_samples, "accuracy": acc, "mean_logit_margin": mean_margin})
...
def make_keybinding_builders(tokenizer, pool: TokenPool, filler_mode: str):
...
def make_counting_builders(tokenizer, steps_max: int, target_char: str = "a"):
    ...
    # Many tokenizers do not represent integers as single tokens (especially with leading spaces).
    # We therefore choose an auto mapping:
    #   - numeric if we have *complete* 0..max_n coverage with single tokens
    #   - otherwise a single-token alphanumeric codebook (still a valid slot-population test)
    count_map, count_mode = build_count_token_map_auto(tokenizer, max_n=steps_max + 5)
    try:
        print(f"[counting_case_study] count_token_mode={count_mode} (max_n={steps_max + 5}, covered={len(count_map)})")
    except Exception:
        pass
    ...
    def build_batch(k: int, b: int, rng: random.Random):
        ...
        src_pos0 = None
        for _ in range(b):
            ids, src_pos, y_tok = build_counting_ids(
                tokenizer, spec, count_map, steps=k, rng=rng, target_char=target_char
            )
            ids_list.append(ids)
            labels.append(y_tok)
            if src_pos0 is None:
                ...
                if src_pos != src_pos0:
                    # Should be fixed for fixed k because all dynamic string parts (step indices) are deterministic.
    ...
    def build_pair(k: int, rng: random.Random):
        ...
        # Not minimal-corruption, but keeps each prompt internally consistent.
        idsA, srcA, yA = build_counting_ids(tokenizer, spec, count_map, steps=k, rng=rng, target_char=target_char)
        # Resample until label differs (needed for probe direction patching)
        for _ in range(50):
            idsB, srcB, yB = build_counting_ids(tokenizer, spec, count_map, steps=k, rng=rng, target_char=target_char)
            if yB != yA:
                break
        else:
            # If we couldn't find a different label, just return (may reduce usefulness of causal patching)
            idsB, srcB, yB = build_counting_ids(tokenizer, spec, count_map, steps=k, rng=rng, target_char=target_char)
...
def main():
    ...
    if "keybinding" in tasks:
        ...
        pool = build_value_token_pool(tokenizer, num_values=args.num_values, max_value_len=args.max_value_len)
        build_batch, build_pair = make_keybinding_builders(tokenizer, pool, filler_mode=args.filler_mode)

        if not args.skip_behavior:
            df_beh = run_behavior_curve(model, build_batch, distances, args.n_behavior, args.batch_size, args.seed + 1)
